...
- Every family child FK includes family_id. No edits to applied migrations0001–0005/0004/0005.
...
- Create `packages/database/migrations/0006_onboarding.sql`.
...
- Helper owner NOLOGIN BYPASSRLS has only narrow SELECT on membership/links/claims and SELECT(id,family_id,version) on members for claim freshness, no login/member grant to runtime, no auth/contact-value privileges. Functions return booleans, derive actor from local GUC, have fixed search_path and PUBLIC execute revoked. They cannot take an arbitrary actor argument that bypasses current actor.
