...
`POST /change-requests` nhận union `member_create`, `relationship_create`, `relationship_update`, `relationship_remove`. `member_create` chỉ nhận hồ sơ tối thiểu và một quan hệ với Member neo; không nhận contact hoặc biography. Khi admin duyệt, Member chưa liên kết tài khoản và Relationship được tạo trong cùng transaction; lỗi ở bất kỳ bước nào đều rollback cả hai. Update chỉ đổi subtype hoặc ngày partnership; đổi đầu cạnh/loại quan hệ dùng remove rồi create để audit rõ. Admin duyệt/từ chối qua `/decision`; người gửi xem pending của mình bằng `scope=mine` và hủy qua `/cancel`. Same-origin JSON, rate limit, version, cùng nhà, duplicate và cycle đều được kiểm phía server; approval kiểm lại trong transaction có khóa theo nhà.

Nhánh web `feat/interactive-family-tree` dùng `GET /relationships` cho sơ đồ quanh hồ sơ đã liên kết, `GET /members/{id}` cho hồ sơ đã lọc contact và toàn bộ workflow đề xuất. Người dùng có thể chọn hồ sơ sẵn có hoặc đề xuất hồ sơ mới, xem lại, gửi, xem pending, hủy, sửa subtype và đề xuất gỡ quan hệ. Quản trị viên xem rồi duyệt/từ chối; graph chính thức chỉ thay đổi sau approval.
