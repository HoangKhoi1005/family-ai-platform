...
Đã chọn PostgreSQL theo [ADR-001](decisions/ADR-001-monorepo.md). [Migration 0001](../packages/database/migrations/0001_identity.sql) chỉ tạo users, family_spaces, family_memberships, members, member_account_links và member_contacts; các bảng khác dưới đây chưa triển khai. RLS hiện deny-all cho runtime, chưa có policy authenticated user. Không dùng FAMILY_001 làm khóa hardcode. Thời điểm hệ thống lưu UTC; ngày lịch lưu kiểu date/field riêng. Bảng có nội dung chỉnh sửa dùng created_at, updated_at, version; audit lưu actor và thời điểm server.
