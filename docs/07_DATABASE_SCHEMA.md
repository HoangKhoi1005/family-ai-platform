...
Migrations `0015_notification_delivery.sql`–`0018_notification_cancel_helper.sql` tạo preference riêng theo membership, inbox source và outbox chỉ giữ tham chiếu Event/Occurrence. Composite foreign key khóa recipient và nguồn vào cùng `family_id`; unique key chống nhân bản theo recipient, occurrence, revision, offset, kind và channel. Queue có due/available/expires, attempts và lease metadata để hai worker dùng `FOR UPDATE SKIP LOCKED` mà không cùng claim một job.

Runtime chỉ đọc và cập nhật preference/inbox của chính active membership, không có quyền đọc hoặc cập nhật trực tiếp outbox. Enqueue dùng context `calendar_write` và helper có tham số hẹp để kiểm recipient active, Event/Occurrence hiện hành, creator/admin cùng nhà, offset được Event bật và push đã được người nhận đồng ý. Việc hủy job cũ cũng đi qua helper riêng thay vì cấp quyền đọc queue. Admin không được đọc inbox của thành viên khác chỉ vì có vai trò admin. Worker credential và thao tác delivery được triển khai ở lát kế tiếp; không dùng owner connection làm runtime production.
